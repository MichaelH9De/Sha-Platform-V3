...
export async function exchangeAndValidateCode(code: string, verifier: string, expectedNonce: string, redirectUri: string) {
  ...
  const { keys } = await jwksResponse.json() as { keys: Array<JsonWebKey & { kid?: string }> };
  const jwk = keys.find((key) => key.kid === header.kid);
  if (!jwk) throw new Error("ID token signing key was not found");
  const validSignature = verify("RSA-SHA256", Buffer.from(`${encodedHeader}.${encodedPayload}`), createPublicKey({ key: jwk, format: "jwk" }), Buffer.from(encodedSignature, "base64url"));
