...
export async function getOidcMetadata(): Promise<OidcMetadata> {
  ...
  if (!response.ok) {
    throw new Error(
      "Unable to load Entra OpenID configuration"
    );
  }
...
export function createSessionToken(
  identity: Omit<SessionIdentity, "exp">
): string {
...
export function verifySessionToken(
  ...
  const identity =
    parsePart<SessionIdentity>(payload);
...
export async function exchangeAndValidateCode(
  code: string,
  verifier: string,
  ...
  if (!tokens.id_token) {
    throw new Error(
      "Entra response did not include an ID token"
    );
  }
  ...
  if (tokenParts.length !== 3) {
    throw new Error("Malformed Entra ID token");
  }
  ...
  const claims =
    parsePart<IdTokenClaims>(encodedPayload);

  if (header.alg !== "RS256" || !header.kid) {
    throw new Error(
      "Unsupported ID token signature"
    );
  }
  ...
  if (!jwksResponse.ok) {
    throw new Error(
      "Unable to load Entra signing keys"
    );
  }

  const { keys } =
    (await jwksResponse.json()) as {
      keys: MicrosoftRsaJwk[];
    };
  ...
  if (!jwk) {
    throw new Error(
      "ID token signing key was not found"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "ID token signing key is not a valid RSA JWK"
    );
  }
  ...
  if (!email) {
    throw new Error(
      "The Entra identity has no email address"
    );
  }
